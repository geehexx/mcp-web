...
"""
    ...
    fm = create_frontmatter(
        "model_decision", "Apply for git operations commits branching or version control work"
    )
    pm = create_postmatter(
        "09_git_workflows.md",
        "model_decision",
...
"""
    ...
    fm = create_frontmatter(
        "model_decision", "Apply at session end when completing work or managing work transitions"
    )
    pm = create_postmatter(
...
def create_13_workflow_routing():
    ...
    content = content.replace(
        "# Workflow Routing Matrix", "# Workflow Routing and Signal Detection"
    )
...
def create_15_tool_patterns():
    ...
    fm = create_frontmatter(
        "model_decision", "Apply when using MCP tools or needing guidance on tool calling patterns"
    )
    pm = create_postmatter(
        "15_tool_patterns.md",
        "model_decision",
