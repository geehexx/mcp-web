...
def create_postmatter(file_name: str, trigger: str, tokens: int, topics: list, 
                       workflows: list = None, dependencies: list = None, 
...
def create_00_core_directives():
    ...
    postmatter = create_postmatter(
        ...
        "always_on",
        3000,
        ["Agent persona and role", "Guiding principles (security, robustness, performance)", 
         "Operational mandate", "Tool selection (uv, pytest, ruff)", "Parallel tool call efficiency"],
...
def create_stub(file_name: str, trigger: str, description: str = None, globs: str = None,
                tokens: int = 2000, topics: list = None):
...
def main():
    ...
    glob_rules = [
        ("01_python_code.md", "*.py, **/*.py", "Python code style type hints async patterns", 
         ["Python style", "Type hints", "Async/await"]),
        ("02_testing.md", "tests/**/*.py, test_*.py, *_test.py, conftest.py", 
         "Testing standards pytest fixtures TDD practices",
         ["pytest", "Test fixtures", "TDD"]),
        ("03_documentation.md", "docs/**/*.md, *.md, README.md",
         "Documentation standards markdown ADRs initiatives",
         ["Markdown", "ADRs", "Initiatives"]),
        ("04_config_files.md", "pyproject.toml, *.ini, Taskfile.yml, .pre-commit-config.yaml",
         "Configuration file best practices TOML YAML Taskfile",
         ["TOML/pyproject.toml", "Taskfile", "Pre-commit"]),
        ("05_windsurf_structure.md", ".windsurf/**/*.md, .windsurf/**/*.json",
