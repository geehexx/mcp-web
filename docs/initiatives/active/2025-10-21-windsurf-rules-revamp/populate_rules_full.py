...
def create_postmatter(
    file_name, trigger, tokens, topics, workflows=None, deps=None, changelog=None
):
    sections = [
        "\n---\n",
        "## Rule Metadata\n",
...
def create_01_python_code():
    ...
    content = content.replace(
        "# Rule: Python Standards and Best Practices", "# Python Code Standards"
    )

    fm = create_frontmatter(
        "glob", "Python code style type hints async patterns and best practices", "*.py, **/*.py"
    )
    pm = create_postmatter(
        "01_python_code.md",
        "glob",
        2200,
        [
...
def create_02_testing():
    ...
    content = content.replace(
        "# Rule: Testing, Tooling, and Development Environment", "# Testing Standards"
    )
...
def create_03_documentation():
    ...
    fm = create_frontmatter(
        "glob", "Documentation standards markdown ADRs initiatives", "docs/**/*.md, *.md, README.md"
    )
    pm = create_postmatter(
        "03_documentation.md",
        "glob",
...
def create_05_windsurf_structure():
    ...
    content = content.replace(
        "# `.windsurf/` Directory Structure", "# Windsurf Directory Structure"
    )
