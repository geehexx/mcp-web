...
pytestmark = pytest.mark.skipif(
    not llm_available,
    reason=f"No LLM available for testing (checked: Ollama, OpenAI, LM Studio)"
)
...
def verify_summary(summary: str, expected: dict) -> tuple[bool, List[str]]:
    ...
    if len(summary) < expected.get("summary_min_length", 0):
        failures.append(
            f"Summary too short: {len(summary)} < {expected['summary_min_length']}"
        )
    
    if len(summary) > expected.get("summary_max_length", 100000):
        failures.append(
            f"Summary too long: {len(summary)} > {expected['summary_max_length']}"
        )
...
class TestGoldenSummarization:
    ...
    async def test_simple_article_summary(
        self, extractor, chunker, summarizer
    ):
    ...
    async def test_technical_doc_summary(
        self, extractor, chunker, summarizer
    ):
    ...
    async def test_news_article_summary(
        self, extractor, chunker, summarizer
    ):
    ...
    async def test_blog_post_summary(
        self, extractor, chunker, summarizer
    ):
...
class TestQueryFocusedSummarization:
    ...
    async def test_query_focused_async_article(
        self, extractor, chunker, summarizer
    ):
    ...
    async def test_query_focused_api_docs(
        self, extractor, chunker, summarizer
    ):
...
class TestMapReduceSummarization:
    ...
    async def test_map_reduce_large_document(
        self, extractor, chunker, summarizer_config
    ):
        ...
        try:
            ...
            for keyword in SIMPLE_ARTICLE_EXPECTED["summary_must_contain"]:
                assert keyword.lower() in summary.lower(), \
                    f"Map-reduce lost keyword: {keyword}"
        
        finally:
            await summarizer.close()
    
    async def test_map_reduce_preserves_structure(
        self, extractor, chunker, summarizer_config
    ):
...
class TestDeterminism:
    ...
    async def test_deterministic_summary(
        self, extractor, chunker, summarizer_config
    ):
        ...
        try:
            ...
            similarity_ratio = len(set(summaries[0].split()) & set(summaries[1].split())) / \
                               max(len(summaries[0].split()), len(summaries[1].split()))
...
class TestLocalLLMSupport:
    ...
    async def test_ollama_summarization(self, extractor, chunker):
        ...
        try:
            ...
            assert any(
                kw in summary.lower() 
                for kw in ["async", "python", "code", "program"]
            )
